...
└── CMakeLists.txt           # -I${CMAKE_CURRENT_SOURCE_DIR}/custom_epilogue（或等价）
```
...
在 **`op_kernel/CMakeLists.txt`** 的 `add_ops_compile_options(... OPTIONS ...)` 中，为 **本算子目录** 增加 `-I`（与现有 `-I${CATLASS_DIR}/include` 并列），例如：

```cmake
OPTIONS -I${CATLASS_DIR}/include -DCATLASS_ARCH=2201 -I${CMAKE_CURRENT_SOURCE_DIR}/custom_epilogue)
```

`${CMAKE_CURRENT_SOURCE_DIR}` 为该算子 `op_kernel/` 目录。若工程用 `target_include_directories`，则等价添加 `${CMAKE_CURRENT_SOURCE_DIR}/custom_epilogue`。
...
- [ ] `op_kernel/CMakeLists.txt` 已包含 `custom_epilogue` 的 `-I`
